...
export const useParakeet = () => {
  ...
  const startConversation = useCallback(async () => {
    ...
    try {
      // Request microphone permissions upfront before making any API calls.
      ...
      sessionPromise.current = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: 'You are Parakeet, a friendly and helpful AI assistant. Keep your responses concise and conversational.',
        },
        callbacks: {
            onopen: () => {
                // Now that the session is open and we have the media stream, connect them.
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (message.serverContent?.turnComplete) {
                ...
                if(currentOutputTranscription.current.trim()) {
                    setTranscriptionHistory(prev => [...prev, { speaker: 'Parakeet', text: currentOutputTranscription.current.trim() }]);
                }
                currentInputTranscription.current = '';
                currentOutputTranscription.current = '';
                setInterimTranscription('');
      ...
      let errorMessage = `Failed to start: ${err.message}`;
      // Provide a more specific error message for permission denial.
